...
void RoutesDataBase::BuildAllRoutes(
...
void RoutesDataBase::AddRoute(
        ...
        const std::shared_ptr<IRouteInfo> &route_info
) {
...
void DirectRoute::check_end_stops() const {
    if (!end_stops_) {
        end_stops_ = std::make_unique<std::vector<std::string>>();
        end_stops_->reserve(2);
        end_stops_->push_back(*stops_.cbegin());

        if (*stops_.cbegin() != *stops_.crbegin())
            end_stops_->push_back(*stops_.crbegin());
    }
}
...
void RoundRoute::check_end_stops() const {
    if (!end_stops_) {
        end_stops_ = std::make_unique<std::vector<std::string>>();
        end_stops_->reserve(1);
        end_stops_->push_back(*stops_.cbegin());
    }
}


void IRouteInfo::Build(
        const std::shared_ptr<BusStopsDataBase> &stops_database,
        const std::shared_ptr<NavigationDataBase> &navigation_database,
        const std::string &route_name
) {
...
bool IRouteInfo::IsEndStop(const std::string &stop) const {
    check_end_stops();
    for (const auto &end_stop : *end_stops_) {
        if (stop == end_stop)
            return true;
    }
    return false;
