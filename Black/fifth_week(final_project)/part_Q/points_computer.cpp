...
namespace Map {
    ...
    void PointsComputer::updateX(
        ...
        if (sorting_vec->size() == 1) {
            sorting_vec->front().x_ = master_->render_settings_->padding_;
        } else {
            const auto max_index = updateSortingVec(sorting_vec);
            const double x_step = max_index == 0 ? 0 :
                                  (master_->render_settings_->width_ - 2 * master_->render_settings_->padding_) /
                                  max_index;
            for (auto &container : *sorting_vec) {
                container.x_ = container.index_ * x_step + master_->render_settings_->padding_;
            }
        }
    }
    ...
    void PointsComputer::updateY(
        ...
        if (sorting_vec->size() == 1) {
            sorting_vec->front().y_ = master_->render_settings_->height_ - master_->render_settings_->padding_;
        } else {
            const auto max_index = updateSortingVec(sorting_vec);
            const double y_step = max_index == 0 ? 0 :
                                  (master_->render_settings_->height_ - 2 * master_->render_settings_->padding_) /
                                  max_index;
            for (auto &container : *sorting_vec) {
                container.y_ = master_->render_settings_->height_ -
                               master_->render_settings_->padding_ - container.index_ * y_step;
            }
        }
    }
