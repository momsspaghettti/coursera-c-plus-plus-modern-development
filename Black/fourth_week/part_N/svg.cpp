...
namespace Svg {
    ...
    Rect &Rect::SetCorner(Svg::Point corner) {
        left_top_corner_ = corner;
        return *this;
    }
    ...
    void Rect::Render(std::ostream &out) const {
        out << "<rect ";
        SvgBase<Rect>::Render(out);

        out << R"(x=\")" << left_top_corner_.x << R"(\" y=\")" << left_top_corner_.y << R"(\" )";
        out << R"(width=\")" << width_ << R"(\" height=\")" << height_ << R"(\" )";
        out << " />";
    }
